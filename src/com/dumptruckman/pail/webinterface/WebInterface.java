...
package com.dumptruckman.pail.webinterface;
...
public class WebInterface {
    ...
    public WebInterface (int port, Pail pail) {
        this.port = port;
        this.pail = pail;
    }

    public WebInterface(Pail pail) {
        this.pail = pail;
        port = 42424;
    }
    ...
    public void webLog(Socket client, String message) {
        ...
        pail.webLogAdd(message);
        // Log message to file
        File logFile = new File("pailweb.log");
        if (!logFile.exists()) {
            try {
    ...
    private String processData(Socket client, String json) {
        ...
        try {
            ...
            // Get console output
                ...
                if (data.isEmpty()) {
                    response = new ResponseWriter("Success", "Got output")
                            .add("Data", pail.getConsoleOutput())
                            .getResponse();
                }
                ...
                // Removes all html formatting except <br>
                    ...
                    response = new ResponseWriter("Success", "Got output with "
                            ...
                            pail.getConsoleOutput(Pail.OutputFormat.LINEBREAK))
                            .getResponse();
                }
                    ...
                    if (data.size() > 1) {
                        if (data.get(1).equalsIgnoreCase("CRLF")) {
                            response = new ResponseWriter("Success", "Got plain"
                                    ...
                                    pail.getConsoleOutput(Pail.OutputFormat
                ...
                // Invalid output format type
                    ...
                    response = new ResponseWriter("Partial Success", 
                            ...
                            pail.getConsoleOutput()).getResponse();
                }
            }
            // Execute task by name
                ...
                java.util.List<String> data = getRequestData(jp);
                if (!data.isEmpty()) {
                    if (pail.startTaskByName(data.get(0))) {
            ...
            // Get a list of task names
                ...
                Iterator it = pail.config.schedule.getEvents().iterator();
                while (it.hasNext()) {
            ...
            // Get task detail by name
                ...
                java.util.List<String> data = getRequestData(jp);
                if (!data.isEmpty()) {
                    EventModel event = pail.getTaskByName(data.get(0));
                    if (event != null) {
